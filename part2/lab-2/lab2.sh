...
read -p "Would you like to change first sentences letters? (y/n)" answer

if [[ "$answer" == "y" ]]; then 
    if [ -e "output_sentences.txt" ]; then
        > output_sentences.txt
    fi
    cat "$filename" | sed -E ':a; N; $!ba; s/(^|[.?!]\s*)([a-zа-я])/\1\U\2/g' >> output_sentences.txt
fi
